...
import type { ServerSettings } from '@/lib/desktop'
...
function toForm(config: ServerSettings): FormState {
  return {
    server: {
...
export function DesktopSettingsPage() {
  ...
  const handleApply = async () => {
    ...
    setSaving(true)
    const nextConfig: ServerSettings = {
      server: {
        ...config.server,
    ...
    <div className="min-h-screen bg-background p-3 sm:p-4 md:p-6">
      <div className="mx-auto max-w-4xl space-y-4">
        <div className="flex flex-wrap items-center justify-between gap-4">
          <div>
            <h1 className="text-xl font-semibold text-foreground">{t('serverSettings')}</h1>
            <p className="text-xs text-muted-foreground">{t('serverSettingsDesc')}</p>
          </div>
